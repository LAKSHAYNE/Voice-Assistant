    ...
    def process_text(self, input):
        try:
            if 'search' in input or 'play' in input:
                ...
                speak = "I have been created by Group-9 members which comprise of Ginni Lakshay LAkshay and Mani."
                self.assistant_speaks(speak)
                return
                ...
                try:
                    app_id = "2Y723Y-V9LV23EALL"  # my wolframaplha id
                ...
                try:
                    print('herererr')
                    app_id = "2Y723Y-V9LV23EALL"  # my wolframaplha id
                    s='http://api.wolframalpha.com/v1/spoken?appid='+app_id+'&i='+input
                    res=requests.get(s)
                    #client = wolframalpha.Client(app_id)
                    #res = client.query(input)
                    #print('>>>>',next(res.results).text)
                    if(res.status_code!=200):
                        self.assistant_speaks("I don't know about it, I can search the web for you, Do you want to continue?")
                        ans = self.get_audio()
                        if 'yes' in str(ans) or 'yeah' in str(ans):
                            self.search_web(input)
                        return
                    answer = str(res.content.decode('utf-8'))
                    self.assistant_speaks(answer)
                    return
                except BaseException as e:
                    Gui.assistant_text(self, e)
